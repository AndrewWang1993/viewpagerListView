...
public class InterceptorFrameLayout extends FrameLayout {
	...
	/** 存放view的左上角的x和y坐标 */
	static int[] viewLocation = new int[2];
	...
	public static boolean isTouchInView(MotionEvent ev, View view) {
		...
		// 返回是否在范围内
		return motionX >= viewLocation[0]
				&& motionX <= (viewLocation[0] + view.getWidth())
				&& motionY >= viewLocation[1]
				&& motionY <= (viewLocation[1] + view.getHeight());
	}
